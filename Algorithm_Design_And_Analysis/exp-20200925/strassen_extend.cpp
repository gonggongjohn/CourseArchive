...
Matrix matProduct(Matrix* matA, Matrix* matB) {
    if ((*matA).row == 1 && (*matA).column == 1 && (*matB).row == 1 && (*matB).column == 1) {
        ...
        matR.m[0][0] = (*matA).m[0][0] * (*matB).m[0][0];
        return matR;
    }
    ...
    Matrix m1 = matProduct(&tmp1, &tmp2);
    Matrix m2 = matProduct(&tmp3, &b11);
    Matrix m3 = matProduct(&a11, &tmp4);
    Matrix m4 = matProduct(&a22, &tmp5);
    Matrix m5 = matProduct(&tmp6, &b22);
    Matrix m6 = matProduct(&tmp7, &tmp8);
    Matrix m7 = matProduct(&tmp9, &tmp10);
